...
printf '%s device=%s mode=%s temp=%s diskFreeMb=%s memoryUsageMb=%s memoryTotalMb=%s cpuLoadAvg=%s uptimeSeconds=%s setup=%s heartbeat=%s kiosk=%s cache=%s updater=%s commandExecutor=%s watchdog=%s\n' "$(date -Is)" "$DEVICE_ID" "$MODE" "$TEMP" "$DISK_FREE_MB" "$MEMORY_USAGE_MB" "$MEMORY_TOTAL_MB" "$CPU_LOAD_AVG" "$UPTIME_SECONDS" "$SERVICE_SETUP" "$SERVICE_HEARTBEAT" "$SERVICE_KIOSK" "$SERVICE_CACHE" "$SERVICE_UPDATER" "$SERVICE_COMMAND_EXECUTOR" "$SERVICE_WATCHDOG" >> "$LOG_DIR/heartbeat.log"
