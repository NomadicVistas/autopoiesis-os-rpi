...
- Use `--list-gates` when CI, staging artifact generation, or rollout tooling needs the authoritative gate catalog. It prints JSON with gate name, order, source environment variable, checker script, and label, and exits without loading manifests, running checkers, or writing readiness reports.
- Use `--manifest-template` when CI needs a complete manifest skeleton from that same catalog. The template disables every source by default and includes each gate's source env, checker, and label as metadata so backend jobs can fill only the evidence they own, enable those gates, and then run `--plan` or the full suite.
...
- Use `--plan` when CI or rollout tooling needs the redacted gate/source matrix before running individual checkers. Plan mode validates the manifest and requirements, fails missing required sources, and records source-present gates as `planned` without fetching or executing those gate sources.
- Set `AUTOPOIESIS_HOSTED_CONTRACT_REPORT=/path/to/hosted-contract-report.json` when CI or staging needs a machine-readable readiness artifact. The report is written on pass, fail, and plan mode, includes `status`, `mode`, `exitCode`, required gates, summary counts, failed gate/reason when available, and per-gate source-presence booleans, and deliberately omits raw source paths, URLs, tokens, and local appliance paths.
