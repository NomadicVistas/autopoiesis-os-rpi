...
Hardware diagnostics read `/proc/device-tree/model` by default, report model, architecture, RAM, support tier, and optional `vcgencmd get_throttled` power/thermal state. Set `AUTOPOIESIS_DEVICE_TREE_MODEL_PATH` and `AUTOPOIESIS_VCGENCMD_BIN` for tests. Raspberry Pi 5 is reported as `recommended`, Raspberry Pi 4 as `supported_baseline`, and Pi 3 or older as `underpowered`. Health emits `hardware_underpowered`, `hardware_low_ram`, `hardware_unknown`, `hardware_undervoltage`, and `hardware_throttled` issue codes when applicable. `scripts/hardware-profile-check.sh` validates diagnostics, health, readiness, and support-bundle propagation; `scripts/hardware-profile-fixture-check.sh` starts the real local UI against fake device-tree and `vcgencmd` inputs to prove Pi 5, Pi 4, Pi 3, and throttling classifications. Physical Milestone 2 runs the live hardware check with `AUTOPOIESIS_REQUIRE_SUPPORTED_HARDWARE=1`.
...
- For strict user-preference coverage, include a nested `userPreferences` flow with `userPreferencesRead`, `newerPreferenceWrite`, `stalePreferenceWrite`, `finalPreferencesRead`, and `heartbeat`, then run with `AUTOPOIESIS_REQUIRE_SETTINGS_USER_PREFERENCES=1`.
...
- Nested user-preference evidence uses the same timestamp rules against `aos_frame_user_preferences`; its heartbeat section should expose effective settings at least as current as the accepted user preference write so cascade freshness is proven.
...
`scripts/settings-contract-check.sh` validates this read-only bundle. It closes the gap between device-side newest-`updatedAt` behavior and durable hosted `aos_` settings rows before heartbeat, Profile > Frames, or Admin > Frames evidence is trusted. When `AUTOPOIESIS_REQUIRE_SETTINGS_USER_PREFERENCES=1` is set, the same gate also requires separate user-preference conflict evidence instead of letting device settings stand in for account-level preferences.
...
Each Profile/Admin device row must also include `actionAvailability` (or `availableActions`/`remoteActionAvailability`) with one explicit allow/deny decision for every supported command. Allowed target decisions for risky commands must mirror the global authorization, audit-id, and local-confirmation flags from `adminFrames.remoteActions`; denied target decisions must include a short reason/disabledReason and may include a stable reason code such as `offline`, `remote_disabled`, `device_disabled`, `subscription_inactive`, `role_denied`, `not_paired`, `pending_command`, or `local_confirmation_required`. This is the target-specific layer: the role matrix says what an actor can do in principle, while `actionAvailability` says whether this exact frame can receive that action right now. The checker applies the same policy comparison to Profile-owned devices and Admin fleet devices so owner-facing controls cannot advertise weaker safety metadata than fleet controls.
...
- `AUTOPOIESIS_REQUIRE_SETTINGS_USER_PREFERENCES=1 scripts/settings-contract-check.sh ...` additionally requires account-level preference conflict evidence and heartbeat cascade freshness.
...
- The local feed preserves redacted polling/freshness hints from hosted stream responses and returns them through `POST /local/feed/sync`, `GET /local/feed`, `GET /local/frame-state`, diagnostics, support bundles, and `feed_synced` delivery evidence.
- `POST /local/heartbeat` evaluates that saved polling policy before sending the heartbeat. If no feed has synced yet, or if `nextPollAt`, `pollAfterSeconds`, `maxPollSeconds`, or `staleAfter` mark the stream due/stale after `minPollSeconds` is respected, the local UI refreshes the stream first and records the local feed poll result in device state.
...
- Use `--list-gates` when CI, staging artifact generation, or rollout tooling needs the authoritative gate catalog. It prints JSON with gate name, order, source environment variable, checker script, label, and dependency list, and exits without loading manifests, running checkers, or writing readiness reports.
- Use `--manifest-template` when CI needs a complete manifest skeleton from that same catalog. The template disables every source by default and includes each gate's source env, checker, label, and dependencies as metadata so backend jobs can fill only the evidence they own, enable those gates, and then run `--plan` or the full suite.
- Use `--strict` for staging or CI jobs that must provide every source before physical Pi acceptance.
- Use `AUTOPOIESIS_HOSTED_CONTRACT_REQUIRE=migrations,schema,pairing,device-auth,settings,profile-ownership,heartbeat,command-poll,command-ack,command-state,stream,cache,online-admin,broadcast,release,release-rollout` when a partial job should require only selected gates while still running any other provided sources. Required-gate names are normalized and typo-checked before gate execution begins.
...
- Manifest source containers (`sources`, `contracts`, `gates`, and `contractSources`) are linted before plan or execution. Unknown source gate keys, non-object source containers, empty string sources, and enabled object entries without `source`, `path`, `file`, or `url` fail early; intentionally disabled entries should use `false` or `{ "enabled": false }`.
- Use `--plan` when CI or rollout tooling needs the redacted gate/source matrix before running individual checkers. Plan mode validates the manifest and requirements, fails missing required sources, records source-present gates as `planned` without fetching or executing those gate sources, and reports advisory `dependencyBlockers` when a required downstream gate lacks upstream evidence.
- Use `--require-dependencies`, `AUTOPOIESIS_HOSTED_CONTRACT_REQUIRE_DEPENDENCIES=1`, or manifest `requireDependencies: true` for staging readiness and rollout manifests that must reject downstream evidence until its prerequisite gates are also sourced or required.
- Set `AUTOPOIESIS_HOSTED_CONTRACT_REPORT=/path/to/hosted-contract-report.json` when CI or staging needs a machine-readable readiness artifact. The report is written on pass, fail, and plan mode, includes `status`, `mode`, `exitCode`, required gates, summary counts, dependency blockers, failed gate/reason when available, and per-gate source-presence booleans, and deliberately omits raw source paths, URLs, tokens, and local appliance paths.
