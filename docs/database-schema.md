...
- `scripts/settings-contract-check.sh` expects durable `aos_frame_device_settings` writes to use newest-`updatedAt` conflict handling. Staging evidence should show an accepted newer settings write, a rejected or explicitly conflicted stale write, a final read preserving the newer row, and heartbeat settings at least as current as the accepted write. Set `AUTOPOIESIS_REQUIRE_SETTINGS_USER_PREFERENCES=1` to require the same conflict sequence for `aos_frame_user_preferences` plus heartbeat evidence that accepted user preference changes cascade into effective device settings.
...
- `scripts/online-admin-contract-check.sh` expects the hosted Profile > Frames surface to derive owned devices from `aos_frame_devices`, user preferences from `aos_frame_user_preferences`, liked artworks from the canonical artwork-like table or an `aos_` mirror, active artists from canonical artist rows plus preference selections, explicit cache preferences from user/device settings, and per-device action availability from the same authorization/device-state evaluator used by Admin > Frames. The online-admin gate treats those profile lists as joined state: profile device ownerUserId must match profileFrames.userId, Profile action availability for risky commands must mirror the same authorization/audit/local-confirmation flags as Admin remote command policy, cache fields mirrored in preferences must match explicit cache preferences, active artist ids must be unique and consistent with `preferences.activeArtists`, and liked artwork ids must be unique with page totals covering returned rows.
- The Admin > Frames portion should derive users, subscribers, subscriptions, fleet devices, global remote action policies, and target-specific action availability from durable `aos_` rows plus the canonical account/subscription models. The online-admin gate now rejects orphaned subscriber rows, subscriptions whose users are absent, entitled subscriptions without subscriber rows, device owners absent from the admin user page, duplicate ids, device subscription ids absent from the subscription page, subscriber summaries that point at another user's subscription, and fleet-device subscription summaries that do not belong to the device owner or drift from referenced status/plan/tier.
...
- `scripts/command-state-contract-check.sh` expects hosted command state evidence to show the same `aos_device_commands` row queued before poll, delivered/sent after poll, terminal after acknowledgement, mirrored in `aos_admin_command_audits`, and absent from the next device poll. It closes the outbox transition gap between delivery selection and acknowledgement persistence.
...
- device-specific settings and user-level preferences should either expose separate authoritative updatedAt values or a documented merged timestamp; the hosted settings contract can now require both durable rows to prove they do not silently overwrite each other.
