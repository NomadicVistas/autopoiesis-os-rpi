...
The repo now has individual gates for migration plans, final `aos_` schema shape, pairing lifecycle evidence, device-route authentication evidence, heartbeat/event-ingestion evidence, stream responses, Profile/Admin Frames bundles, broadcast lifecycle evidence, and release manifests. Running them one-by-one is easy to forget and makes backend handoff ambiguous.
...
5. `scripts/heartbeat-contract-check.sh`
6. `scripts/stream-contract-check.sh`
7. `scripts/online-admin-contract-check.sh`
8. `scripts/broadcast-contract-check.sh`
9. `scripts/release-manifest-check.sh`
