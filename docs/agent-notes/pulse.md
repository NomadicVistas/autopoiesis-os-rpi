...
- 503 when token not configured prevents accidental open access in development.
- Admin token is separate from device API key — admin auth does NOT bypass device auth.

**Next:** Configure in production environment, wire into frontend admin dashboard, consider token rotation.
